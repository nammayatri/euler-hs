...
-- | Template-Haskell helpers for wiring a Beam table up to the KV connector.
-- This is a slimmed-down copy of @Kernel.Beam.Lib.UtilsTH@ from shared-kernel,
-- stripped of its @Kernel.*@ dependencies so it can live inside euler-hs.
--
-- The only entry points intended for external use are 'enableKVPG' and
-- 'mkTableInstances'; all other helpers are exposed only because TH splices
-- need them in scope at the call site.
module EulerHS.KVConnector.UtilsTH
  ( enableKVPG,
    HasSchemaName (..),
    mkTableInstances,
...
-- | Encode any ToJSON value to strict Text (JSON).
...
enableKVPG name pKeyN sKeysN = do
  ...
  pure [tModeMeshSig, tModeMeshDec, meshMetaDec, kvConnectorDec, sqlObjectToJSONInstance]
...
sortAndGetKey names = do
  let sortedKeys = DL.sort $ fmap (splitColon . nameBase) names
  DL.intercalate "_" sortedKeys
...
utilTransform modifyMap field value = do
  ...
  case res of
    A.String r -> r
    A.Number n -> T.pack $ P.show n
    A.Bool b -> T.pack $ P.show b
    A.Array l -> T.pack $ P.show l
    A.Object o -> T.pack $ P.show o
    A.Null -> T.pack ""
...
mkSerialInstances name = do
  let putInstance = FunD 'Serialize.put [Clause [] (NormalB (AppE (VarE 'P.error) (LitE $ StringL ""))) []]
      getInstance = FunD 'Serialize.get [Clause [] (NormalB (AppE (VarE 'P.error) (LitE $ StringL ""))) []]
